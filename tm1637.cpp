...
TM1637::TM1637(gpio_num_t clk, gpio_num_t data) {
    clkpin = clk;
    datapin = data;
...
int TM1637::writeByte(int8_t wr_data) {
    for (uint8_t i = 0; i < 8; i++) { // Sent 8bit data
        ...
        if (wr_data & 0x01) {
            gpio_set_level(datapin, 1);    // LSB first
        } else {
    ...
    gpio_set_level(clkpin, 0); // Wait for the ACK
    gpio_set_level(datapin, 1);
    gpio_set_level(clkpin, 1);
    gpio_set_direction(datapin, GPIO_MODE_INPUT);

    bitDelay();
    uint8_t ack = gpio_get_level(datapin);
...
// Send start signal to TM1637 (start = when both pins goes 0)
...
// End of transmission (stop = when both pins goes 1)
...
void TM1637::displayStr(char str[], uint16_t loop_delay) {
		...
		if(end <= DIGITS){
    		...
    		offset++;
    		vTaskDelay(loop_delay/ portTICK_PERIOD_MS); //loop delay
			}
		}
}
